...
script_file_include(lua_State *L) throw()
{
    ...
    GameAgent::agent()->leader()->scriptInclude(Path::dataReadPath(filename));
    END_NOEXCEPTION;
...
script_game_planAction(lua_State *L) throw()
{
    ...
    GameAgent::agent()->leader()->planAction(funcRef);
    END_NOEXCEPTION;
    return 0;
}
//-----------------------------------------------------------------
...
{
    BEGIN_NOEXCEPTION;
    bool planning = GameAgent::agent()->leader()->isPlanning();
